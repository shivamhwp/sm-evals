...
const beirDatasetPath = env.beirDatasetPath;

let limit = 20;
if (env.lgDatasets.includes(datasetName)) {
  console.log(
    "Using default retrieval limit for lg dataset i.e  ",
    env.lgMaxRetrievalLimits
  );
  limit = env.lgMaxRetrievalLimits;
} else if (env.smDatasets.includes(datasetName)) {
  console.log(
    "Using default retrieval limit for sm dataset ",
    env.smMaxRetrievalLimits
  );
  limit = env.smMaxRetrievalLimits;
}
